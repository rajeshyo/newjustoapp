...
if ( ! functionExists( 'validateForm' ) ) {
	ish.validateForm = function() {
		...
		jQuery('#ish-contact-form .required').each(function() {
		...
		// Validate form
		jQuery('#ish-contact-form input[type="text"].required').on('input', function() {
			verifyText( jQuery(this) );
		});

		jQuery('#ish-contact-form input[type="email"]').on('input', function() {
			verifyEmail( jQuery(this) );
		});

		jQuery('#ish-contact-form input[type="url"]').on('input', function() {
			verifyUrl( jQuery(this) );
		});

		jQuery('#ish-contact-form textarea.required').keyup(function() {
			verifyMessage( jQuery(this) );
		});

		// Verify form on submit
		jQuery('#ish-contact-form input[type="submit"], form button[type="submit"]').click(function(e) {
			var thisForm = jQuery(this).closest('#ish-contact-form');
...
/*--------------------------------------------------------------
 ## Validate Form - Warranty Form
 --------------------------------------------------------------*/
if ( ! functionExists( 'validateWarrantyForm' ) ) {
	ish.validateWarrantyForm = function() {

		// Adding stars and attribute "required" for required inputs
		jQuery('#ish-warranty-form .required').each(function() {
			var phValue = jQuery(this).attr('placeholder');
			jQuery(this).attr( 'placeholder', phValue + ' *' );
			jQuery(this).attr( 'required', 'required' );
		});
		...
		// Validate form
		jQuery('#ish-warranty-form input[type="text"].required').on('input', function() {
			verifyText( jQuery(this) );
		});

		jQuery('#ish-warranty-form input[type="email"]').on('input', function() {
			verifyEmail( jQuery(this) );
		});
		
		jQuery('#ish-warranty-form input[type="file"]').on('input', function() {
			verifyFile( jQuery(this) );
		});
		
		$('#ish-wf-purchase').on('change', function() {
		  verifySelect( jQuery(this) );
		});
		
		jQuery('#ish-warranty-form input[type="tel"]').on('input', function() {
			verifyPhone( jQuery(this) );
		});
		
		jQuery('#ish-warranty-form textarea.required').keyup(function() {
			verifyMessage( jQuery(this) );
		});

		// Verify form on submit
		jQuery('#ish-warranty-form input[type="submit"], form button[type="submit"]').click(function(e) {
			var thisForm = jQuery(this).closest('#ish-warranty-form');

			thisForm.find('input.required, textarea.required, select.required').each(function() {
				var thisEl = jQuery(this);
				var type = thisEl.attr('type') || thisEl.localName;
				switch( type ) {
					case 'text':
						verifyText( thisEl );
						break;
			...
			if ( 0 == thisForm.find('.invalid').length ) {
				...
				thisForm.submit(function(event) {
					...
					if ( 'ish-warranty-form' == thisForm.attr('id') ) {
						var url = "sendwarrantymail.php";
		...
		// Tel must be number
		function verifyPhone( thisEl ) {
			var isTel = thisEl.val();
			if ( isTel && isTel.length == 10) {
				thisEl.removeClass("invalid").addClass("valid");
				thisEl.parent().removeClass("required-error");
			} else {
				thisEl.removeClass("valid").addClass("invalid");
		...
		// Verify purchase selected
		function verifySelect( thisEl ) {
			var isSelect = thisEl.val();
			if(isSelect == "") {
				thisEl.removeClass("valid").addClass("invalid");
				thisEl.parent().addClass("required-error");
			}
